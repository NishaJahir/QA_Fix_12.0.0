...
jQuery(document).ready( function() {
    ...
    // validate the customer birthday day
    jQuery("#nn_guarantee_date").on("keypress textInput",function (e) {
        var keyCode = e.which || e.originalEvent.data.charCodeAt(0);
        var expr = String.fromCharCode(keyCode);  
        if(isNaN(expr) || (/android/i.test(userAgent) && jQuery(this).val().length > 1 )) {           
            e.preventDefault();
        }
        var day_val = jQuery('#nn_guarantee_date').val();
        if(day_val.length == 1) {
    ...
    jQuery('#nn_guarantee_date').on('blur', function() {
        var date, updated_date;
        updated_date = date = jQuery('#nn_guarantee_date').val();
        if(date != '0' && date != '' && date.length < 2) {
            updated_date = "0"+ date;         
        } else if(date == '0') {
            updated_date = date.replace('0', '01');        
        } 
        jQuery('#nn_guarantee_date').val(updated_date);
    });      

    // validate the customer birthday year and autoload the year
    jQuery("#nn_guarantee_year").on("input", function(e) {      
        ...
        if(e.keyCode != 8 || e.keyCode != 46) {        
            if(user_val > maximum_year || user_val <  minimum_year || isNaN(user_val))  {        
                jQuery(this).val( year_val.substring( 0, year_len - 1 ) );
                e.preventDefault();
    ...
    function yearAutocomplete(input_val, array_year) {
        ...
        function addActiveValue(x) {
            ...
            jQuery('#nn_guarantee_year').val( jQuery('.year_active', elements).val() );
        }
